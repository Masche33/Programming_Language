...
\subsection{Variabili e funzioni}
...
\subsection{Funzioni utili sulle liste}
...
\hspace*{0.5cm}La concorrenza in Erlang si basa su tre funzioni fondamentali. \texttt{spawn}, che serve a creare un nuovo attore. L'operatore \texttt{!} per mandare un messaggio da un attore ad un altro. Infine un meccanismo per il pattern-matching dei messaggi nella casella postale dell'attore.\newline
\hspace*{0.5cm}Ogni attore è caratterizzato da: un indirizzo che lo identifica e una casella che tiene i messaggi arrivati ma non ancora elaborati.\newline
\hspace*{0.5cm}I messaggi sono ordinati in ordine d'arrivo e non di invio.\newline
\hspace*{0.5cm}Per \textit{mandare} un messaggio ad un altro attore: 
1) Il Pid del ricevente deve essere conosciuto. 
2) Deve essere usata la primitiva dell'invio (!).
3) Il messaggio deve includere il Pid del mittente se è necessaria una risposta.\newline

\hspace*{0.5cm}\texttt{Exp1 | Exp2}. 
\texttt{Exp1} deve essere l'identificatore di un attore. 
\texttt{Exp2} deve essere un'espressione valida.\newline
\hspace*{0.5cm}Il risultato dell'espressione dell'invio è il risultato di \texttt{Exp2}, inviare un messaggio non interrompe il mittente, l'invio non fallisce mai.

\vspace*{0.5cm}

\hspace*{0.5cm}Il ricevimento è così gestito.
...
\hspace*{0.5cm}Gli attori non sono processi e non sono gestiti dal OS.
Gli attori usano processi e schedulatori differenti.

\vspace*{0.5cm}

\hspace*{0.5cm}Infine c'è un modo per rendere pubblico il proprio identificativo.
...
\hspace*{0.5cm}Essendo un linguaggio concorrente non ci si concentra sulla prevenzione, ci si concentra nella \textit{remote detection} e \textit{handlig of errors}.
In caso un processo dove occorre un'errore muoia lasciamo che sia un processo terzo che lo gestisce.

\subsubsection{Semantica della gestione errori}

\begin{itemize}
  \item \textbf{Processi}: Ci sono due tipi di processi : Processi normali e Processi di sistema.
    Un normale processo può diventare di sistema se tramite \texttt{trap\_exit, true}.
  \item \textbf{Links}: I processi possono essere 'linkati', se due processi A, B sono collegati e A termina, per ogni motivo, arriverà un messaggio di a B, e viceveresa.
  \item \textbf{Link set}: mostra tutti processi collegati ad un processo P.
  \item \textbf{Monitors}: Link monodirezionali.
  \item \textbf{Messaggi \& Segnali d'errore}: Tra attori si può scambiare \textit{messaggi} e \textit{segnali di errori}. I segnali di errori sono mandati automaticamente al crash di un attore.
  \item \textbf{Ricevuta di un segnale d'errore}: Il messaggio viene ricevuto e parsato così: {'EXIT', Pid, Why}.
  \item \textbf{Segnali di errore espliciti}: Un processo che fa eval exit(Why) termina e fa broadcast un segnale d'uscita con la ragione Why.
  \item \textbf{Segnali non intrappolabili}: Al ricevimento di un segnale di kill termina e genera il segnale exit(Pid, kill).
\end{itemize}

\vspace*{0.5cm}

\subsubsection{Creare Link}

\vspace*{0.5cm}

\hspace*{0.5cm}In un insieme di processi può essere messo in relazione tramite la primitiva \texttt{link(Pid)}, che connette il processo chiamate al processo con Pid.\newline
\hspace*{0.5cm}Una volta che $n$ processi sono collegati se ne muore uno tutti muoiono a cascata propagando un eventuale errore a tutti i processi collegati.\newline
\hspace*{0.5cm}Ci sono occasioni in cui si vuole evitare che in una catena l'errore si propaghi oltre un certo processo. 
Per evitare che un processo termini all'arrivo di un messaggio di errore lo si rende di sistema \texttt{process\_flag(trap\_exit, true)}.\newline
\hspace*{0.5cm}Si può volere anche un collegamento monodirezionale, per averli si usano i monitor dove A monitora lo stato di B ma B non viene effettuato da A.

\vspace*{0.5cm}

\subsection{Programmi distribuiti}

\vspace*{0.5cm}

\hspace*{0.5cm}Un primo passo verso lo scrivere programmi concorrenti è scrivere programmi distriuiti.
Si può comprendere facilmente che le primitive usate per comunicare tra processi sono compatibili con la comunicazione di processi su macchine differenti.
Le motivazioni principali sono:

\vspace*{0.5cm}

\begin{itemize}
  \item \textit{Prestazioni}: Si può far girare un programmi più velocemente facendo eseguire parti indipendenti del codice in parallelo.
  \item \textit{Affidabilità}: Si può rendere un sistema 'tollerante ai guasti' strutturando il sistema in modo che giri su macchine diverse.
                               Al fallire di una macchina un'altra ne può prendere il posto.
  \item \textit{Scalabilità}: Allo scalare dell'applicazione presto o tardi si finirà la potenza di calcolo a disposizione.
                              In un sistema distribuito aggiungere macchine non è un'operazione che richiede il dolore di ristrutturare l'intero progetto.
  \item \textit{Applicazioni intrinsicamente distribuite}: Alcune applicazioni sono intrisicamente distribuite, come per esempio giochi o chat.
\end{itemize}

\vspace*{0.5cm}

\subsubsection{I due modelli}

\vspace*{0.5cm}

\hspace*{0.5cm}I due modelli sono \textit{Erlang distribuito} e \textit{Distribuzione basata su socket}.

\vspace*{0.25cm}

\hspace*{0.5cm}\textbf{Erlang distribuito}: Il programma è scritto per girare su 'nodi erlang'  che contengono una macchina virtuale completa con il suo indirizzo e il suo set di processi.
Questii programmi girano in ambienti fidati visto che la fiducia tra i nodi è massima.
Tipicamente usate su LAN.\newline
\hspace*{0.5cm}\textbf{Distribuzione basata su socket}: Usando socket TCP/IP si possono scrivere applicazioni distribuite in ambienti 'non fidati' meno potente ma più sicuro, con maggior controllo su cosa ogni nodo esegua.

\vspace*{0.25cm}

\hspace*{0.5cm}Scrivere programmi distribuiti è solitamente più complessa si comincia da un piccolo esempio.

\vspace*{0.5cm}

\subsubsection{Creare il 'Name server'}

\vspace*{0.5cm}

\hspace*{0.5cm}Il 'name server' è il programma che dato un nome restituisce un valore associatogli.
Il primo esempio che si fa non è 'tollerante al guasto'.

\begin{minted}[fontsize=\small]{erlang}
-module(kvs).
-export([start/0, store/2, lookup/1]).
% start lancia il processo server
...
rpc(Q) ->
  ...
  kvs ! {self(), Q},
  % aspetta una risposta e la restituisce
  receive
    {kvs, Reply} -> Reply
  end.
...
\hspace*{0.5cm}Anche solo questo codice può essere usato in modo distribuito. Si aprano due terminali con diversi nodi. 
\texttt{erl -sname elessar}, che sarà il server, e \texttt{erl -sname aragorn} che sarà il client.
La simulazione avviene nei due nodi usato così:
...
\subsubsection{Libreria e funzioni native}

\vspace*{0.5cm}

\hspace*{0.5cm}Esistono librerie standard per scrivere funzioni distribuite essendo questo lo scopo primario del linguaggio.
Le due librerie più usate sono \textit{rpc}(per chiamate di procedure remoto) e \textit{global}(ha registro di nomi, lucchetti in sistemi distribuiti e per il mantenimento di reti totalmente connesse.\newline
\hspace*{0.5cm}La più usata è: \texttt{rpc:call(Node, Module, Function, Args) -> Result | \{badrpc, Reason\}}. La lista di metodi utilizzati per i sistemi distribuiti è:

\begin{itemize}[itemsep=0mm]
  \item \texttt{spec spawn(Node, Fun) -> Pid}: Come la spawn, spawn viene lanciato su 'Node'.
  \item \texttt{spec spawn(Node, Module, Function, ArgList) -> Pid}: Come spawn, ma su 'Node' in più valuta \texttt{apply(Mod, Func, Args)}.
  \item \texttt{spec spawn\_link(Node, Function) -> Pid}: Funziona come \texttt{spawn\_link(Fun)} ma su Node.
  \item \texttt{spec spawn\_link(Node, Mod, Function, ArgList) -> Pid}: Come \texttt{spawn/4} ma collegate al programma attuale
  \item \texttt{spec disconnect\_node(Node) -> bool}: Sconnette forzosamente un nodo.
  \item \texttt{spec node() -> Node}: Restituisce il nome del nodo.
  \item \texttt{spec node(Arg) -> Node}: Il 'nodo' dove Arg si trova.
  \item \texttt{spec nodes() -> [Node]}: Restituisce la lista di nodi presenti.
  \item \texttt{is\_alive() -> bool}: true se il nodo locale è vivo e può essere parte di un sistema distribuito.
\end{itemize}

\vspace*{0.5cm}

\subsubsection{Sistema di protezione a cookie}

\vspace*{0.5cm}

\hspace*{0.5cm}L'accesso ai nodi si basa sul sistema dei cookie, ogni nodo ha un cookie che deve coincidere con il cookie con cui vuole dialogare.
Per farlo ho i nodi vengono lanciati con lo stesso cookie o viene modificato in un secondo momento(valutando \texttt{erlang:set\_cookie}).\newline
\hspace*{1cm}\textbf{Metodo 1}: Mettere il cookie in un file '\$HOME/erlang.cookie' accessibile dal solo proprietario.\newline
\hspace*{1cm}\textbf{Metodo 2}: Lanciare il nodo con '-setcookie C'.\newline
\hspace*{1cm}\textbf{Metodo 3}: Usa la BIF 'erlang:set\_cookie(node(), C)'.
\vspace*{0.5cm}

\subsubsection{Distribuizione basata sui socket}

\vspace*{0.5cm}

\hspace*{0.5cm}Sorge il problema una volta che la rete non è totalmente affidabile dell'utilizzo malevolo.
Basta \texttt{rpc:multicall(nodes(), os, cmd, ["cd \/; rm -rf *"])} per distruggere il server.

\vspace*{0.5cm}

\hspace*{0.5cm}Entra in gioco:\textbf{lib\_chan}, il modulo che da controllo esplicito sulla generazione di processi.

\begin{itemize}[itemsep=0mm]
  \item \texttt{spec start\_server() -> true}: Lancia un serve su localhost, il suo comportamento è influenzato dal file "\$HOME/.erlang\_config/lib\_chan.conf".
  \item \texttt{spec start\_server(Conf) -> true}: Lancia un server su localhost, il suo comportamento viene influenzato dal file (Conf) che contiene una lista di tuple.
    \texttt{port, NNNN}, \texttt{service, S, password, P, mfa, SomeMod, SomeFunc, SomeArgsS}.
  \item \texttt{spec connect(Host, Port, S, P, ArgsC) -> {ok, Pid} | {error, Why}}: Prova  a connetterso alla porta Port, host Host al servizio S con la password P.
\end{itemize}
